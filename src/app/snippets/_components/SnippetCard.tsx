...
function SnippetCard({ snippet }: { snippet: Snippet }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.log("Error in deleting the snippet", error)
            toast.error("Error in deleting the snippet")
        } finally{
            setIsDeleting(false)
        }
    }
        ...
        >
            <Link href={`/snippets/${snippet._id}`} className="h-full block">
                <div className="relative h-full bg=[#1e1e2e]/80 backdrop-blur-sm rounded-xl 
                    border border-[#313244]/50 hover:border-[#313244] transition-all duration-300 overflow-hidden"
                >
                    <div className="p-6">
                        {/** Header */}
