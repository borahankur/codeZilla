...
export const deleteSnippet = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (snippet.userId !== identity.subject) throw new Error("Unauthorized access to delete. User not owner of snippet")
...
export const starSnippet = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("stars")
            .withIndex("by_snippet_id")
            .filter((q) => q.eq(q.field("snippetId"), args.snippetId) && q.eq(q.field("userId"), identity.subject))
            .first()
...
export const deleteComment = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (comment.userId !== identity.subject) throw new Error("User is not authorized to make")
